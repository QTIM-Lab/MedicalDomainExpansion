...
#
#Lambda values for EWC
for lambda_val,lambda_name in zip([.005,.05,.5,1,10,500,1000,100000],['p005','p05','p5','1','10','500','1000','100000']):
	#Repetitions
	for rep in ['1','2','3','4','5']:
		modelname =  'Name to save model as'+lambda_name+rep
		...
		class NetArch(nn.Module):
			...
			def forward(self, x):
				...
				return x
		#
		old_modelname =  #name of model trained on original domain
		PATH = datadir+old_modelname+'.pth.tar'
		#
		# Send model to GPU
		model = NetArch()
		model.load_state_dict(torch.load(PATH))
		#
		#freeze all layers except BN
		...
		#
		model.netarch.apply(freeze_all_but_bn)
		model.to(device)
		#
		...
		lamda = lambda_val
		#
		fishers_cpu = torch.load('path of fisher matrix')
		fishers = []
		for name, fisher in fishers_cpu.items():
			fishers.append(fisher.to(device))
		#
		#Train the model
		for epochID in range(0, trMaxEpoch):
			...
			for batchcount, (varInput, target) in enumerate(dataLoaderTrain):
				...
				model.eval() #Freeze batchnorm statistics during training
				...
				with torch.set_grad_enabled(True): # enable gradient while training
					...
					loss_ewc = ewc_loss(outputs, labels, lamda, fishers, prev_opt_thetas, cur_thetas)
					trainloss = trainloss + loss_ewc
					trainloss.backward()
					optimizer.step()
